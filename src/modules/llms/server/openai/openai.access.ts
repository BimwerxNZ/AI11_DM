...
export type OpenAIAccessDiagnostics = z.infer<typeof openAIAccessDiagnosticsSchema>;
export const openAIAccessDiagnosticsSchema = z.object({
  credentialSource: z.enum(['browser', 'server-env', 'none']),
  hostSource: z.enum(['browser', 'server-env', 'default']),
  orgSource: z.enum(['browser', 'server-env', 'none']),
  browserKeyPresent: z.boolean(),
  serverKeyPresent: z.boolean(),
  browserHostPresent: z.boolean(),
  serverHostPresent: z.boolean(),
  browserOrgPresent: z.boolean(),
  serverOrgPresent: z.boolean(),
  resolvedHost: z.string(),
  usesDefaultOpenAIHost: z.boolean(),
  hostOverrideActive: z.boolean(),
  orgOverrideActive: z.boolean(),
  heliconeEnabled: z.boolean(),
});

export function openAIAccessDiagnostics(access: OpenAIAccessSchema): OpenAIAccessDiagnostics {
  const browserKeyPresent = !!access.oaiKey;
  const browserHostPresent = !!access.oaiHost;
  const browserOrgPresent = !!access.oaiOrg;
  const browserHeliconePresent = !!access.heliKey;

  const serverKeyPresent = !!env.OPENAI_API_KEY;
  const serverHostPresent = !!env.OPENAI_API_HOST;
  const serverOrgPresent = !!env.OPENAI_API_ORG_ID;
  const serverHeliconePresent = !!env.HELICONE_API_KEY;

  let oaiHost: string;
  let credentialSource: OpenAIAccessDiagnostics['credentialSource'];
  let hostSource: OpenAIAccessDiagnostics['hostSource'];
  let orgSource: OpenAIAccessDiagnostics['orgSource'];
  let heliKey: string | false;

  if (browserHostPresent) {
    oaiHost = access.oaiHost;
    credentialSource = browserKeyPresent ? 'browser' : 'none';
    orgSource = browserOrgPresent ? 'browser' : 'none';
    heliKey = browserHeliconePresent ? access.heliKey : false;
    hostSource = 'browser';
  } else {
    oaiHost = env.OPENAI_API_HOST || DEFAULT_OPENAI_HOST;
    credentialSource = browserKeyPresent ? 'browser'
      : serverKeyPresent ? 'server-env'
        : 'none';
    orgSource = browserOrgPresent ? 'browser'
      : serverOrgPresent ? 'server-env'
        : 'none';
    heliKey = access.heliKey || env.HELICONE_API_KEY || false;
    hostSource = serverHostPresent ? 'server-env' : 'default';
  }

  oaiHost = llmsFixupHost(oaiHost, OPENAI_API_PATHS.chatCompletions);
  ...
  return {
    ...
    browserOrgPresent,
    serverOrgPresent,
    resolvedHost: oaiHost,
    usesDefaultOpenAIHost: llmsHostnameMatches(oaiHost, DEFAULT_OPENAI_HOST),
    hostOverrideActive: browserHostPresent || serverHostPresent,
    orgOverrideActive: orgSource !== 'none',
    heliconeEnabled: !!heliKey,
  };
}
